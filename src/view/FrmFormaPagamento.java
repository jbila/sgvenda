...
import com.meldev.dao.FormaDePagamentoDao;
import com.meldev.model.Empresa;
import com.meldev.model.FormasDePagamento;

public class FrmFormaPagamento extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FrmFormaPagamento frame = new FrmFormaPagamento();
	...
	public FrmFormaPagamento() {
		...
		JLabel lblTipoDeDespesa = new JLabel("formaDePagamentoDao");
		...
		txtSearch.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				if (!txtSearch.getText().isEmpty()) {
					List<FormasDePagamento> formasDePagamento = formaDePagamentoDao.findAll(txtSearch.getText(),empresa.getId());
					showInfo(formasDePagamento);
		...
		JLabel lblTipoDeReceita = new JLabel("Forma de Pagamento");
	...
	private void save() {
		...
		formasDePagamento.setEmpresa(empresa);

		if (!nome.isEmpty()) {
			formasDePagamento.setNome(nome);
			formasDePagamento.setDescricao(txtNome.getText()); // Use txtDescricao em vez de txtNome
			if (ID.equals("")) {
				formasDePagamento.setId(null);
			} else {
				formasDePagamento.setId(Long.valueOf(ID));
			}

			FormasDePagamento cat = formaDePagamentoDao.save(formasDePagamento);
	...
	private void delete() {
		if (!txtId.getText().isEmpty()) {
			...
			if (confirmation == JOptionPane.YES_OPTION) {
				formaDePagamentoDao.deleteById(Long.valueOf(txtId.getText()));
				showformaDePagamentoDao();
				clear();
			} else {
	...
	//
	private void showformaDePagamentoDao() {
		
			List<FormasDePagamento> formasDePagamento = formaDePagamentoDao.findAll(empresa.getId());
			showInfo(formasDePagamento);
		
	}

	private void showInfo(List<FormasDePagamento> formasDePagamento) {
		DefaultTableModel model = (DefaultTableModel) table.getModel();
		Object[] row = new Object[3];
		model.setRowCount(0);// empty the table
		for (int i = 0; i < formasDePagamento.size(); i++) {
			row[0] = formasDePagamento.get(i).getId();
			row[1] = formasDePagamento.get(i).getNome();
			model.addRow(row);
		}
