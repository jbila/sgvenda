...
public class FrmDocumento extends JFrame {
	...
	public FrmDocumento() {
		...
		JLabel lblTipoDeDespesa = new JLabel("Documento");
		...
		txtSearch.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				if (!txtSearch.getText().isEmpty()) {
					List<Documento> documentos = documentoDao.findAll(txtSearch.getText(),empresa.getId());
					showInfo(documentos);
		...
		JLabel lblTipoDeReceita = new JLabel("Tipo de Documento");
	...
	private void save() {
		...
		if (!nome.isEmpty()) {
			...
			Documento.setDescricao(txtNome.getText()); // Use txtDescricao em vez de txtNome
			if (ID.equals("")) {
			...
			Documento cat = documentoDao.save(Documento);
	...
	private void delete() {
		if (!txtId.getText().isEmpty()) {
			...
			if (confirmation == JOptionPane.YES_OPTION) {
				documentoDao.deleteById(Long.valueOf(txtId.getText()));
				showDocumento();
				clear();
			} else {
	...
	//
	private void showDocumento() {
		if (empresa != null) {
			List<Documento> documentos = documentoDao.findAll(empresa.getId());
			showInfo(documentos);
		}
	}

	private void showInfo(List<Documento> documentos) {
		DefaultTableModel model = (DefaultTableModel) table.getModel();
		Object[] row = new Object[3];
		model.setRowCount(0);// empty the table
		for (int i = 0; i < documentos.size(); i++) {
			row[0] = documentos.get(i).getId();
			row[1] = documentos.get(i).getNome();
			model.addRow(row);
		}
